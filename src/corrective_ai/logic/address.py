...
def parse_address_components(components: List[Dict]) -> Dict[str, str]:
    ...
    suitable for updating DataFrame columns.
    Handles alphanumeric postal codes safely (NL / UK / CA).
    """
    ...
    address_parts = {
        ...
        "postal_code": "",
        "postal_code_suffix": "",   # ✅ safe to add (no behavior change if unused)
        "country": "",
        "country_code": ""
    }
    ...
    for component in components:
        ...
        if "street_number" in types:
            ...
            # short_name preserves abbreviations (CA, ZH, QC, etc.)
            ...
            address_parts["postal_code_suffix"] = component.get("long_name", "")
    ...
    # ------------------------------------------------------------
    dest_address1 = " ".join(
        filter(None, [address_parts["street_number"], address_parts["route"]])
    ).strip()
    ...
    # ------------------------------------------------------------
    # Construct ZIP / Postal Code safely
    # ------------------------------------------------------------
    zip_code = address_parts["postal_code"].strip()

    if address_parts["postal_code_suffix"]:
        zip_code = f"{zip_code} {address_parts['postal_code_suffix']}".strip()
